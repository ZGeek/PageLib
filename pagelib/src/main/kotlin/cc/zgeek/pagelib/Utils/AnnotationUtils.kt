...
fun ViewWrapper.injectView() {
    ...
    while (clazz != ViewWrapper::class.java) {
        ...
        for (i in 0..fields.size() - 1) {
            ...
            try {
                val view = this.findViewById(resId.toInt())
                if (view == null && PageLibDebugUtils.isDebug) {
                    Log.w(PageLibDebugUtils.TAG, this.javaClass.name + " with property (" + fields.valueAt(i).name + ") will be NULL")
                }
                fields.valueAt(i).set(this, view)
...
fun ViewWrapper.injectLayout(): View? {
    var clazz: Class<*> = this.javaClass
    while (clazz != ViewWrapper::class.java) {
        val id = findLayoutId(clazz, this.resources, this.packageName)
